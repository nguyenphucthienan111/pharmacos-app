...
import {
    View, Text, StyleSheet, ScrollView, ActivityIndicator, TouchableOpacity, Image, Alert, SafeAreaView, Platform
...
const OrderDetailScreen = () => {
                ...
                if (data?.order) {
                    ...
                    const updatedItems = await Promise.all(items.map(async (item) => {
                        if (typeof item.productId === 'string') {
                            ...
                            try {
                                const res = await fetch(ApiEndpoints.PRODUCTS.GET_BY_ID(item.productId));
    ...
    const total = (order.items || []).reduce((sum, item) => sum + (item.unitPrice || item.price || 0) * (item.quantity || 1), 0);

    const shipFee = 1000; // Giả định phí ship cố định như web
    ...
    return (
        <SafeAreaView style={styles.container}>
            <ScrollView contentContainerStyle={{ padding: 0, backgroundColor: '#f4f6fb' }}>
                ...
                <View style={styles.headerBox}>
                    ...
                    <Text style={styles.orderDateHeader}>Date: {order.orderDate ? new Date(order.orderDate).toLocaleString() : '-'}</Text>
                </View>
                ...
                {status !== 'cancelled' && (
                    <View style={styles.estimateBox}>
                        ...
                        <Text style={styles.estimateText}>From <Text style={{ fontWeight: 'bold' }}>11:00 - 12:00</Text> on {order.orderDate ? new Date(order.orderDate).toLocaleDateString() : '-'}</Text>
                        <Text style={styles.estimateSub}>Your order is being processed at PharmaCos.</Text>
                    </View>
                )}
                {/* Thông báo khi đơn bị hủy */}
                {status === 'cancelled' && (
                    <View style={styles.cancelBox}>
                        <Text style={styles.cancelText}>Order cancelled at {order.updatedAt ? new Date(order.updatedAt).toLocaleTimeString() : '-'} on {order.updatedAt ? new Date(order.updatedAt).toLocaleDateString() : '-'}</Text>
                        {order.cancelReason && <Text style={styles.cancelReason}>Reason: {order.cancelReason}</Text>}
                        <Text style={styles.cancelSub}>We hope to serve you again next time.</Text>
                    </View>
                )}
                ...
                <View style={styles.section}>
                    ...
                    {(order.items || []).map((item, idx) => (
                        <View key={item._id || idx} style={styles.productCard}>
                            <Image
                                source={
                                    item.productId?.images && item.productId.images.length > 0 && item.productId.images[0].url
                            ...
                            <View style={{ flex: 1, marginLeft: 14 }}>
                                ...
                                <Text style={styles.productDesc}>{item.productId?.description || ''}</Text>
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6 }}>
                                    <Text style={styles.productPrice}>{formatVND(item.unitPrice || item.price || 0)}</Text>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Shipping fee</Text><Text style={[styles.summaryValue, { color: '#1976d2' }]}>{formatVND(shipFee)}</Text></View>
                    <View style={styles.summaryRowTotal}><Text style={styles.summaryLabelTotal}>Grand total</Text><Text style={styles.summaryValueTotal}>{formatVND(grandTotal)}</Text></View>
                </View>
...
const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#f4f6fb', paddingTop: Platform.OS === 'android' ? 0 : 0 },
    ...
    sectionTitle: { fontWeight: 'bold', fontSize: 17, marginBottom: 10, color: '#1976d2' },
    infoText: { fontSize: 15, color: '#333', marginBottom: 2 },
    productCard: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 14, padding: 12, marginBottom: 12, alignItems: 'center', elevation: 3, borderWidth: 1, borderColor: '#e3e3e3', shadowColor: '#1976d2', shadowOpacity: 0.07, shadowRadius: 6 },
    productImage: { width: 60, height: 60, borderRadius: 10, backgroundColor: '#eee' },
    ...
    productQty: { color: '#fff', backgroundColor: '#1976d2', borderRadius: 8, paddingHorizontal: 10, marginLeft: 12, fontWeight: 'bold', fontSize: 15, overflow: 'hidden' },
    totalText: { color: '#1677ff', fontWeight: 'bold', fontSize: 20 },
    backButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1976d2', borderRadius: 24, paddingVertical: 14, paddingHorizontal: 28, elevation: 2 },
    backButtonText: { color: '#fff', fontWeight: 'bold', marginLeft: 10, fontSize: 17 },
    estimateBox: { backgroundColor: '#e6f0fa', borderRadius: 12, padding: 16, marginBottom: 14, marginHorizontal: 12 },
    estimateTitle: { fontWeight: 'bold', fontSize: 16, color: '#1976d2', marginBottom: 4 },
    estimateText: { fontSize: 15, marginBottom: 2, color: '#1976d2' },
    estimateSub: { color: '#888', fontSize: 14 },
    cancelBox: { backgroundColor: '#fff3f3', borderRadius: 12, padding: 16, marginBottom: 14, marginHorizontal: 12, borderWidth: 1, borderColor: '#ffd6d6' },
    cancelText: { fontSize: 15, marginBottom: 2, color: '#d32f2f', fontWeight: 'bold' },
    cancelReason: { color: '#d32f2f', fontSize: 14, fontWeight: '500', marginTop: 8 },
    cancelSub: { color: '#888', fontSize: 14, marginTop: 4 },
    buyAgainButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1677ff', borderRadius: 24, paddingVertical: 14, paddingHorizontal: 28, elevation: 2 },
    buyAgainButtonText: { color: '#fff', fontWeight: 'bold', marginLeft: 10, fontSize: 17 },
    ...
    infoLabel: { fontWeight: 'bold', color: '#1976d2', marginBottom: 2, fontSize: 15 },
    ...
    buttonRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 16, paddingHorizontal: 12, marginBottom: 32, marginTop: 8 },
});
