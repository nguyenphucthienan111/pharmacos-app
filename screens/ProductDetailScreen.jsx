...
import {
  View, Text, StyleSheet, ScrollView, Image, TouchableOpacity, ActivityIndicator, FlatList, Alert, Modal, TextInput
...
const StarRating = ({ rating = 0, size = 18, onRate, disabled = false }) => (
  <View style={{ flexDirection: 'row' }}>
    {[1, 2, 3, 4, 5].map((star) => (
      <TouchableOpacity key={star} onPress={() => onRate && onRate(star)} disabled={disabled}>
        <AntDesign name={rating >= star ? 'star' : 'staro'} size={size} color="#FFC107" />
      </TouchableOpacity>
    ))}
  </View>
);
...
const ReviewFormModal = ({ visible, onDismiss, onSubmit, initialReview, product, isLoading }) => {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onDismiss}>
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>{initialReview ? 'Edit Your Review' : `Review "${product?.name}"`}</Text>
...
const ProductDetailScreen = ({ route }) => {
  ...
  const handleSubmitReview = async (reviewData) => {
    ...
    if (result.success) {
      Alert.alert("Success", `Your review has been ${userReview ? 'updated' : 'submitted'}!`);
  ...
  if (loading) {
    return <ActivityIndicator style={styles.centerScreen} size="large" color={colors.primary} />;
  }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'ingredients':
        return <Text style={styles.tabContentText}>{product.ingredients?.join(', ') || 'No ingredients listed.'}</Text>;
      case 'reviews':
        return (
          <View>
            ...
            {product.reviews && product.reviews.length > 0 ? product.reviews.map(review => (
              <View key={review._id} style={[styles.reviewCard, review._id === userReview?._id && styles.userReviewCard]}>
                <View style={styles.reviewHeader}>
                  <Text style={styles.reviewUser}>{review.userId?.profile?.name || 'Anonymous'}</Text>
  ...
  return (
    <>
      <ScrollView style={styles.container}>
        <Image source={{ uri: product.images[selectedImageIndex] || 'https://via.placeholder.com/400' }} style={styles.mainImage} />
        {product.images.length > 1 && (
          <FlatList horizontal data={product.images} renderItem={({ item, index }) => (
            <TouchableOpacity onPress={() => setSelectedImageIndex(index)}>
              <Image source={{ uri: item }} style={[styles.thumbnail, selectedImageIndex === index && styles.activeThumbnail]} />
            </TouchableOpacity>
        ...
        <View style={styles.infoContainer}>
          ...
          <View style={styles.priceContainer}>
            <Text style={styles.price}>{discountedPrice ? discountedPrice.toLocaleString() : product.price.toLocaleString()} VND</Text>
            {discountedPrice && <Text style={styles.originalPrice}>{product.price.toLocaleString()} VND</Text>}
          </View>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <ProductCard product={{ ...item, imageUrl: item.images?.find(i => i.isPrimary)?.url || item.images?.[0]?.url }} />}
...
const styles = StyleSheet.create({
  ...
  thumbnail: { width: 70, height: 70, borderRadius: 8, marginRight: 10, borderWidth: 2, borderColor: 'transparent' },
  activeThumbnail: { borderColor: '#006782' },
  infoContainer: { padding: 20 },
  brand: { fontSize: 16, color: '#006782', marginBottom: 4, fontWeight: '500' },
  productName: { fontSize: 24, fontWeight: 'bold', color: '#222', marginBottom: 8 },
  ...
  originalPrice: { fontSize: 16, color: '#999', textDecorationLine: 'line-through', marginLeft: 10 },
  tabsContainer: { flexDirection: 'row', justifyContent: 'space-around', borderBottomWidth: 1, borderBottomColor: '#eee' },
  ...
  bottomBar: { flexDirection: 'row', alignItems: 'center', padding: 10, borderTopWidth: 1, borderTopColor: '#eee', backgroundColor: '#fff' },
  favoriteButton: { padding: 10 },
  quantitySelector: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#ddd', borderRadius: 8, marginHorizontal: 10 },
  quantityButton: { fontSize: 20, paddingHorizontal: 15, paddingVertical: 10, color: '#555' },
  quantityText: { fontSize: 18, fontWeight: 'bold', paddingHorizontal: 20, color: '#333' },
  addToCartButton: { flex: 1, backgroundColor: '#006782', padding: 15, borderRadius: 8, alignItems: 'center' },
  ...
  writeReviewButton: { alignSelf: 'center', backgroundColor: '#e6f7ff', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 20, marginBottom: 20 },
  writeReviewButtonText: { color: '#006782', fontWeight: 'bold' },
  similarProductsTitle: { fontSize: 20, fontWeight: 'bold', paddingHorizontal: 20, marginTop: 10, marginBottom: 10 },
  ...
  reviewInput: { height: 100, textAlignVertical: 'top', borderColor: '#ddd', borderWidth: 1, borderRadius: 8, padding: 10 },
